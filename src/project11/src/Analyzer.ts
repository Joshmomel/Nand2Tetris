...
class Analyzer {
  ...
  private generateCode() {
    ...
    if (isDirectory) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (fileArray[1] === 'jack') {
          try {
            ...
            const complicationEngine = new CompilationEngine()
            const parentNode = complicationEngine.analyse(tokens)!
            const fileName = path.split('.')[0] + '.xml'

            fs.closeSync(fs.openSync(fileName, 'w'))
            fs.unlinkSync(fileName)
            printTreeString(parentNode, 0, fileName)
            Analyzer.writeToFile(path, complicationEngine.getVm().toString(), '.vm')
          } catch (err) {
      ...
      try {
        ...
        fs.unlinkSync(fileName)
        printTreeString(parentNode, 0, fileName)
        Analyzer.writeToFile(filePath, complicationEngine.getVm().toString(), '.vm')
      } catch (err) {
        new Error('generate code error')
      }
    }
  ...
  private static writeToFile(filePath: string, content: string, fileType = '.xml') {
    const fileName = filePath.split('.')[0] + fileType
